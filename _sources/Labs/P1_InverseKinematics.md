...
- Write a Python script that picks up blocks from known locations and stack them at a different location.
...
- Open `proj1_fucntions.py` and copy everthing except the `test_inverse_kinematics` function and paste it in your `xarm.py` at the end of `class XArm(rbt.DHRobot)` right before 
```Python
""" ======================================================================
    End of Class XArm
    ====================================================================== """
...
**Note:** Frame {5} is attached at the tooltip because of a bug in robotics-toolbox.  It cannot render the end-effector link correctly if we use `robot.tool = SE3([0, 0, L4])`
...
- In your `invkine()`, you must check the operating ranges of joint angles and should not return solutions that violate the oerating ranges. Ensure you have the following two lines inside the constructor of XArm (`def __init__(...)`) and use the `check_joint_angle_limits` method to check the ranges before returning from `invkine()`.
...
Write a Python script that moves a cube located at (25, 0, 0) to (20, 16, 0) and demo your robot moving the cube.  You must use the inverse kinematics, not the forward kinematics with pre-calculated joint angles. You must lift up the block no less than 10 cm before placing it down. You may be asked to move the cube from a difference location. 
